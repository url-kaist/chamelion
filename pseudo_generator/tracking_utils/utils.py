...
def load_vertex(scan_path):
    ...
    if scan_path.endswith(".npy"):
        current_vertex = np.load(scan_path)
        current_vertex = current_vertex[:, :4]
        current_vertex[:, 3] = 1.0
        return current_vertex
    elif scan_path.endswith(".pcd"):
        import pcl

        current_vertex = pcl.load(scan_path)
        current_vertex = current_vertex.to_array()
        current_vertex = np.concatenate(
            (current_vertex, np.ones((current_vertex.shape[0], 1))), axis=1
        )
        # current_vertex = current_vertex[:, :4]
        current_vertex[:, 3] = 1.0
        return current_vertex
...
def transform_point_cloud(pcd, pose):
    ...
    """
    xyz1 = np.hstack([pcd, np.ones((pcd.shape[0], 1))]).T
    transformed_xyz = np.dot(pose, xyz1).T[:, :3]
